    ...
    public class DeliveryController : Controller
    {
        ...
        public ActionResult Confirmation(int id, DeliveryConfirmation confirmation)//Suvidha CR
        {
            ...
            using (var scope = Context.Read())
            {
                ...
                if (Session["AlreadyUpdated"] == null)
                    Session["AlreadyUpdated"] = false;

                if (Convert.ToBoolean(Session["AlreadyUpdated"]) == false)
                {
                    Session["AlreadyUpdated"] = true;
                    foreach (var shipment in shipments)
                    {
                        if (shipment.DeliveryOrCollection != "D")
                        {
                            string cosacsAccountId = shipment.AcctNo;
                            ProcessDeliveryRepository processDeliveryRepository = new ProcessDeliveryRepository();
                            string invoiceNumber = processDeliveryRepository.IsMambuAccount(cosacsAccountId);
                            if (invoiceNumber != "")
                            {
                                ProcessDeliveryCommand processDeliveryCommand = new ProcessDeliveryCommand
                                {
                                    CosacsAccountId = cosacsAccountId,
                                    InvoiceNumber = invoiceNumber
                                };
                                string result = ProcessDelivery.PostProcessDelivery(processDeliveryCommand);
                                //if (result != "Success")
                                //{
        ...
        public JsonResult ConfirmSingleShipment(int shipmentId, string confirmedDate, string rejectionReason, string rejectionNotes, int? quantity)
        {
            ...
            using (var scope = Domain.Context.Write())
            {
                ...
                var shipment = (from b in scope.Context.Booking
                 ...
                 cosacsAccountId = shipment.AcctNo;
